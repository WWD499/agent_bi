...
>
> **当前版本：v1.0.0（首个正式里程碑，Tag `v1.0.0`）** ｜ 后端 Spring Boot 3.4.5 / Java 21 ｜ 前端 Vue 3 + Vite 6 ｜ 已接入 GitHub Actions CI 门禁（build + test）。
...
| **查询历史** | 按用户隔离的查询记录（SQL / 耗时 / 返回行数 / 状态），支持回溯与复现分析过程。 |
| **站内信通知** | 预警与系统事件经站内信中心推送（`bi_notify` 表）；邮件通道在配置 `spring.mail.host` 时并行发送，否则自动降级为站内信 + 日志。 |
...
- **服务端强制确认护栏（P0）**：沙箱写工具（建表 / 改名改表 / 导入 / 落表 CTAS / 删表）虽由前端弹确认框，但服务端以 `trusted-mode` 二次校验——即便客户端伪造 `skipConfirm`，服务端仍按 `trusted-mode=false` 视为需确认，从根本上杜绝绕过确认直接落库。
...
后端单元测试（JUnit 5 + Mockito）覆盖预警规则、SQL 校验、图表选型、多级缓存、大屏 CRUD、知识库、确认流与 ReAct 边界、沙箱列名解析等核心逻辑：
...
当前共 **19 个测试类 / 124 个用例（@Test 方法）**，全部通过。

> **CI 门禁**：仓库已接入 GitHub Actions，push / PR 到 `main` 会跑后端 `mvn verify` + 前端 `npm run build` + 前后端版本号一致性校验，任一失败阻断合入。
