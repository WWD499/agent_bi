...
> 文档版本：v2.1（2026-08-04 修订）｜ 编制：架构组 ｜ 配套：需求规格说明书、功能详细设计文档、详细技术实现方案
> **v2.1 修订要点**：废弃「ry 系统库 + 独立 MySQL 业务库」双库方案，改为**单库 `agent_bi`**（独立于 ai_bi 共用的 `ry` 库），系统表与多域业务星型表同库共存；初始化脚本见 `bi-agent-platform/sql/agent_bi_init.sql`。新增数据沙箱三表（bi_sandbox_db / bi_sandbox_table / bi_sandbox_audit）及 ER 关系。
...
| 库 | 类型 | 用途 | 管理方式 |
|----|------|------|----------|
| **PostgreSQL `agent_bi`** | 系统主库 + 业务库（单库双用） | 同时存储 BI 平台元数据/配置/知识库向量/预警数据 **与** 多域业务星型表（dim_*/fact_*） | 由 `sql/agent_bi_init.sql` 一次性建表灌数；应用启动时读取 |
...
> 以上业务表均位于 PostgreSQL `agent_bi` 库（非 MySQL）。初始化 SQL 见 `bi-agent-platform/sql/agent_bi_init.sql`，重跑即清空重建。
