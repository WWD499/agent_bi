...
> 文档版本：v2.0（2026-07-16 修订）｜ 编制：架构组 ｜ 配套：需求规格说明书、功能详细设计文档、详细技术实现方案
> **v2.0 修订要点**：废弃「ry 系统库 + 独立 MySQL 业务库」双库方案，改为**单库 `agent_bi`**（独立于 ai_bi 共用的 `ry` 库），系统表与多域业务星型表同库共存；初始化脚本见 `bi-agent-platform/sql/agent_bi_init.sql`。
